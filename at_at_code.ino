...
double[] singleLegAngles(int coordinates[2]) {
  double C_factor = 3.8298;
  double q1, q2;
  double angles[2];

  q2 = acos((coordinates[0] * coordinates[0] + coordinates[1] * coordinates[1] - LEG_LENGTH * LEG_LENGTH - LEG_LENGTH * LEG_LENGTH) / (2 * LEG_LENGTH * LEG_LENGTH));
  q1 = atan2(coordinates[1], coordinates[0]) - atan2(LEG_LENGTH * sin(q2), LEG_LENGTH + LEG_LENGTH * cos(q2));

  angles[1] = 180 - ((90 + degrees(q1)) * C_factor);
  angles[2] = 180 - ((90 + degrees(q1) + degrees(q2)) * C_factor);

  return angles;
}

void testMoveSingleLeg(int legId, double angles[2]) {
  dxl.setGoalPosition(2*legId-1, angles[0], UNIT_DEGREE);
  dxl.setGoalPosition(2*legId, angles[1], UNIT_DEGREE);
}
